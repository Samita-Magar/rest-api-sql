...
exports.authenticateUser = async (req, res, next) => {
  ...
  if (credentials) {
    // Attempt to retrieve the user from the data store by their emailAddress
    ...
    if (user) {
      // Use the bcrypt.js npm package to compare the user's password
      // (from the Authorization header) to the user's password
      // that was retrieved from the data store.
      const authenticated = bcrypt
        .compareSync(credentials.pass, user.password); // returns true or false
